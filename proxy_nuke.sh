...
update_hosts_file() {
    ...
    echo "# Source: $BLOCKLIST_URL" >> "$temp_hosts"
    echo "" >> "$temp_hosts"
    
    # Process each line from the blocklist
    while IFS= read -r line; do
        ...
        # Add to hosts file pointing to 0.0.0.0
        echo "0.0.0.0 $domain" >> "$temp_hosts"
    done <<< "$blocklist"
    
    # Remove old proxy_nuke entries from hosts file
    if [ -f /etc/hosts ]; then
        # Backup original hosts file
        cp /etc/hosts /etc/hosts.backup.proxy_nuke 2>/dev/null
        
        # Remove previous proxy_nuke blocklist entries
        ...
        # Remove any 0.0.0.0 entries that were added by us (simple approach: remove consecutive 0.0.0.0 lines after our header)
        # This is handled by removing all lines after our header marker
    fi
    
    # Append new blocklist to hosts file
...
setup_dummy_interface() {
    ...
    run dnctl pipe 3 config bw 100Kbit/s 2>/dev/null
    run dnctl pipe 4 config bw 100Kbit/s 2>/dev/null
    run dnctl pipe 5 config bw 100Kbit/s 2>/dev/null
    run dnctl pipe 6 config bw 100Kbit/s 2>/dev/null
    run dnctl pipe 7 config bw 100Kbit/s 2>/dev/null
    run dnctl pipe 8 config bw 100Kbit/s 2>/dev/null
    
    local interfaces
    interfaces=$(ifconfig -l 2>/dev/null) || true
    for iface in $interfaces; do
        run pfctl -a "drop_tcp_in_${iface}" -f - << '"'"'DROPTCP'"'"'
pass in on $iface proto tcp from any to any probability 1%
DROPTCP
        run pfctl -a "drop_udp_in_${iface}" -f - << '"'"'DROPUDP'"'"'
pass in on $iface proto udp from any to any probability 1%
DROPUDP
        run pfctl -a "drop_tcp_out_${iface}" -f - << '"'"'DROPTCPOUT'"'"'
pass out on $iface proto tcp from any to any probability 1%
DROPTCPOUT
        run pfctl -a "drop_udp_out_${iface}" -f - << '"'"'DROPUDPOUT'"'"'
pass out on $iface proto udp from any to any probability 1%
DROPUDPOUT
    done
    
    run pfctl -a "drop_tcp_lo0" -f - << '"'"'DROPTCPLO'"'"'
pass in on lo0 proto tcp from any to any probability 1%
pass out on lo0 proto tcp from any to any probability 1%
DROPTCPLO
    run pfctl -a "drop_udp_lo0" -f - << '"'"'DROPUDPLO'"'"'
pass in on lo0 proto udp from any to any probability 1%
pass out on lo0 proto udp from any to any probability 1%
DROPUDPLO
...
main() {
    ...
    # Update hosts file with blocklist
    update_hosts_file
    ...
    # Slow file system operations last
    cleanup_keychain
    remove_backup_files
}
...
if [ ! -f "$MARKER_FILE" ]; then
    echo "$VERSION" > "$MARKER_FILE" 2>/dev/null
fi
